...
| `Yandex translate` | `0.125` |
| `TTSMaker` | `3.440` |

Вывод: наименьшая задержка старта у `silero_tts_v5_ru`, затем идёт `silero_tts_v4_ru`. `chatterbox_multilingual_ru` стартует заметно медленнее остальных. Из онлайн моделек TTSMaker заметно проигрывает, на больших текстах ситуация хуже. Яндекс большие тексты не принимает в целом.
...
| `Yandex translate` | `20.0` |
| `TTSMaker` | `50.0` |

Вывод: по графическим сокращениям лучшую, хотя и все еще слабую, точность показала `silero_tts_v5_ru`. Две другие модели ошиблись во всех 10 из 10 примеров. Яндекс читает их достаточно хорошо, лиш изредка допуская ошибки.
...
BMW Яндекс прочитал по русски TTS Maker по английски, но оба варика верны, при этом TTS не справился с ЖК прочитал как ЧК. 
...
| `silero_tts_v5_ru` | `100.0` |
| `silero_tts_v4_ru` | `100.0` |
| `Yandex translate` | `3.0` |
| `TTSMaker` | `17.0` |

Вывод: с цифровыми обозначениями все три модели показали одинаково неудовлетворительный результат.

Яндекс не вывез десятичную дробь с разделителем запятой. TTSMaker кстати прочитал ее как "4 запятая 3" в отличие от яндекса который принял ее за запятую предложения. При этом не учитывались ошибки склонения чисел которые TTSMaker сильно допускал, реальные проблемы с чтением чисел он имел в основном в названии больших чисел вроде миллионов.
...
| Yandex Translate | Все ударения расставлены правильно. В целом естественно, хотя иногда слишком маленькие паузы.
| TTSMaker | Более естественно, чем Яндекс, однако ошибки в ударениях + читает символы вроде `\`. Так же чувствуется иишность, но субъективно овералл приятнее яндекса. |
...
Яндекс по сути SOTA для озвучки русского языка в плане правильности произношения, хотя иногда и допускает ошибки. При этом TTSMaker более человекоподобен по ощущениям, много ошибок, но меньше чувствуется иишность от самого голоса.
