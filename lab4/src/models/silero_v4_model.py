...
def validate_config(config: dict) -> None:
    """Проверяет конфигурацию модели Silero V4.
...
def load_model(config: dict) -> object:
    ...
    try:
        ...
        raise RuntimeError("Для Silero V4 нужен torch 2.0+") from error
...
def prepare_text(config: dict, text: str) -> str:
    """Подготавливает текст для Silero V4.
    ...
    Fallbacks:
        Модель использует собственные автоударения для ru, поэтому спецобработка минимальна.
    """
...
def synthesize(model_handle: object, config: dict, text: str, output_path: str) -> dict:
    """Синтезирует wav через Silero V4.
